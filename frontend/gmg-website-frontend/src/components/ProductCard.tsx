import { Button, CartButton } from "./Button/Button";
      ...
      <div className="relative flex-1 bg-white shadow-md mx-3 lg:mx-6 p-3 -mt-8 text-black">
        <div className="flex flex-col gap-2">
          <h2 className="text-xs lg:text-2xl font-bold">
            <Link to={`/detail-product/${uuid}`} className="text-amber-800">{productName}</Link>
          </h2>
          ...
          <div className="flex-col lg:flex text-sm lg:text-2xl text-amber-500">
            {isFlashSale ? (
              <div>
                ...
                PHP {discount.toLocaleString("en-PH")}
              </div>
            ) : (
              <span className="text-s lg:text-md">{`Php. ${price?.toLocaleString(
                "id"
              )} `}</span>
            )}
          </div>
